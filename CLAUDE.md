...
- The pieces this fork draws itself — the decorations (`APT_MODULAR_*`) and the mirrored small terminal and small runway — are gated by the `station.new_airport_graphics` setting; `AreNewAirportGraphicsAvailable()` is the gate and `IsNewAirportGraphicsPiece(piece, rotation)` says which pieces it covers. Only the decorations are refused by the single-tile build command: the mirrored *orientation* of a legacy piece is what an already-rotated layout may contain, so that command does not inspect rotation. Template placement is narrower: while the setting is off, a template containing a small runway stays limited to 0/180-degree rotation; with it on, the mirrored sprites permit all four rotations. The settings window greys the setting out while `station.modular_airports` is off, but that dependency lives only in `SettingDesc::IsEditable` — no command consults `modular_airports`. `_settings_game` is all-false in the unit tests, so a test that builds a decoration must set the flag itself.
