...
Regression: `scripts/regression_test.sh` runs three saves under headless 5-year simulations and compares total airport movements against the committed floors in `scripts/testdata/*.expected`. Run it after any change to reservation, pathfinder, or movement code. Per-commit attribution is `scripts/airport_stats_history.sh`. See `CLAUDE.md` for the fixture-by-fixture detail and the caveats about paused saves and log paths.
