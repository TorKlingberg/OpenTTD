...
static void PlaceAirport(TileIndex tile)
{
	...
	auto proc = [=](bool test, StationID to_join) -> bool {
		if (_build_airport_as_modular && airport_type < NEW_AIRPORT_OFFSET) {
			if (test) {
				return Command<Commands::BuildModularAirportFromStock>::Do(CommandFlagsToDCFlags(GetCommandFlags<Commands::BuildModularAirportFromStock>()), tile, airport_type, layout, StationID::Invalid(), adjacent).Succeeded();
			} else {
				return Command<Commands::BuildModularAirportFromStock>::Post(STR_ERROR_CAN_T_BUILD_AIRPORT_HERE, CcBuildAirport, tile, airport_type, layout, to_join, adjacent);
			}
		} else {
			if (test) {
	...
	void OnPaint() override
	{
		...
		if (_selected_airport_index != -1) {
			...
			if (_settings_game.economy.infrastructure_maintenance) {
				Money monthly = ScaleAirportMaintenanceCost(_price[Price::InfrastructureAirport], as->maintenance_cost * 8);
	...
	NWidget(WWT_PANEL, Colours::DarkGreen),
		NWidget(NWID_VERTICAL), SetPIP(0, WidgetDimensions::unscaled.vsep_normal, 0), SetPadding(WidgetDimensions::unscaled.picker),
			NWidget(NWID_VERTICAL), SetPIP(0, WidgetDimensions::unscaled.vsep_picker, 0),
				...
				EndContainer(),
			EndContainer(),
			NWidget(NWID_SELECTION, Colours::Invalid, WID_AP_MODULAR_SEL),
				NWidget(NWID_VERTICAL),
					NWidget(WWT_LABEL, Colours::Invalid), SetStringTip(STR_STATION_BUILD_AIRPORT_MODULAR_LABEL), SetFill(1, 0),
					NWidget(NWID_HORIZONTAL), SetPIP(14, 0, 14), SetPIPRatio(1, 0, 1),
						NWidget(NWID_HORIZONTAL, NWidContainerFlag::EqualSize),
							NWidget(WWT_TEXTBTN, Colours::Grey, WID_AP_BTN_NOTMODULAR), SetMinimalSize(60, 12), SetFill(1, 0),
														SetStringTip(STR_STATION_BUILD_AIRPORT_MODULAR_OFF, STR_STATION_BUILD_AIRPORT_MODULAR_OFF_TOOLTIP),
							NWidget(WWT_TEXTBTN, Colours::Grey, WID_AP_BTN_MODULAR), SetMinimalSize(60, 12), SetFill(1, 0),
...
{
	return ShouldDrawSavedTemplatePreviewAtTileInternal(tile);
}
