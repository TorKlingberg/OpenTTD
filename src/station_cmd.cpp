...
/* StationNaming enum is in station_cmd.h */
 ...
 * @param filter Optional per-tile filter; tiles it rejects are not part of the catchment
 * @return Array of production per cargo type.
 */
CargoArray GetProductionAroundTiles(TileIndex north_tile, int w, int h, int rad, CatchmentTileFilter filter)
{
	CargoArray produced{};
	...
	for (TileIndex tile : ta) {
		if (filter != nullptr && !filter(tile)) continue;
 ...
 */
std::pair<CargoArray, CargoTypes> GetAcceptanceAroundTiles(TileIndex center_tile, int w, int h, int rad, CatchmentTileFilter filter)
{
...
static bool ClickTile_Station(TileIndex tile)
{
	...
	if (bst->facilities.Test(StationFacility::Waypoint)) {
		...
		if (st->airport.blocks.Test(AirportBlock::Modular)) {
			ShowDepotWindow(tile, VehicleType::Aircraft);
		} else {
